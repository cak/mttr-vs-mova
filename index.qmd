...
::: {.shock-subtitle}
Dashboards signal failure here.
The first question is: did we slow down, or did we change what we fixed?

What happened to backlog age, and to the oldest open work?
:::
::::
...
::: {.line-anchor}
MTTR increased after we launched a vulnerability management program.
:::

::: {.incremental}
- MTTR became a KPI
- MTTR went up
- Review question: “What changed?”
- Dashboard read: the vulnerability management program regressed
:::
...
::: {.metric-big .centered}
The MTTR Paradox: MTTR rises while backlog age falls
:::
...
We needed a way to measure the risk still open, not just what closed.
:::
...
::: {.kicker}
Backlog age. Current age of open vulnerabilities.
:::
...
::: {.definition-card}
### Backlog
Open vulnerabilities not yet remediated
:::

::: {.definition-card}
### Backlog age
How long that work has been open
:::

::: {.definition-card}
### Exposure
Aging backlog increases exposure over time.
:::
:::

::: {.compact-list}
- MTTR and MOVA use the same source data
- Open findings age from the created date
...
Fix arrivals, remediation capacity, and time horizon.
:::
...
Same system. Same inputs. Only prioritization changes.
:::
...
(
    ...
    + geom_line(size=3.0)
    + geom_point(data=mttr_labels, size=4.4)
    + geom_label(
        data=mttr_labels,
...
(
    ...
    + geom_line(size=3.0)
    + geom_point(data=mova_labels, size=4.4)
    + geom_label(
        data=mova_labels,
        mapping=aes(y="label_y", label="label"),
        ha="left",
