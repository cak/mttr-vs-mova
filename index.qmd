...
We do not prioritize by age. We prioritize by risk. Metrics show the consequences of those decisions.
:::
:::
...
- Teams started closing older findings first
...
Older vulnerabilities often persist because they are harder to fix, require coordination, or carry higher risk.
:::
...
Each answers a different question. Neither is sufficient alone.
...
::: {.tail-card .emphasis}
::: {.tail-text .strong}
The backlog said “absolutely not.”
:::
::: {.tail-text}
...
:::
### Prioritization
Two simplified closure patterns.

Only the order of closure changes so the metric behavior is visible.
:::
:::
...
Oldest-First produces lower backlog age in the simulation, but it ignores risk and is not a real-world strategy.
...
(
    strategy_line_plot(
        dataframe=mttr_plot,
        y_col="days",
        title="Newest-First looks better on MTTR alone",
...
::: {.centered .muted}
On MTTR alone, Newest-First looks better.
...
```{python}
#| echo: false
#| fig-alt: "Line chart of MOVA over time under two prioritization strategies. Reaching older backlog work lowers MOVA because older findings are being closed instead of left to age."
...
(
    strategy_line_plot(
        dataframe=mova_plot,
        y_col="days",
        title="Older backlog work lowers MOVA",
        subtitle="MOVA falls when the team reaches older backlog work.",
...
::: {.centered .muted}
MOVA shows whether older backlog is being reached.
:::

---

## The 180+ tail shows stranded backlog

```{python}
#| echo: false
#| fig-alt: "Line chart of open vulnerabilities older than 180 days under two prioritization strategies. The 180+ tail shrinks only when older backlog work is actually being reached."
...
(
    strategy_line_plot(
        dataframe=tail_plot,
        y_col="aged_over_180",
        title="The 180+ tail shows stranded backlog",
        subtitle="If the tail does not shrink, older backlog is being left behind.",
...
::: {.centered .muted}
The tail shows whether older backlog is being reached.
:::
...
decision_table = (
    summary.with_columns(
        ...
        .then(pl.lit("Lower backlog age"))
        .otherwise(pl.lit("Lower MTTR"))
        .alias("decision"),
        pl.when(pl.col("strategy") == "oldest_first")
        .then(pl.lit(0))
        .otherwise(pl.lit(1))
...
(
    ...
    .tab_header(
        ...
        subtitle="MTTR reflects recent closures. MOVA reflects current backlog age. Neither is sufficient alone.",
    )
...
## MTTR and MOVA answer different questions {.center}
...
::: {.line-anchor .centered .blood-line}
Both are partial signals. The mistake is reading either one alone.
:::
...
::: {.centered .muted}
If this is the only KPI, recent closures can look like full-program progress.
:::
...
::: {.centered .muted}
The fuller view shows flow, backlog age, size, and tail risk together.
:::
...
## Prioritization is risk-based {.center}

Real teams should not remediate by age alone.
...
Not newest-first or oldest-first.
...
Backlog
:::
### Open count + aging tail
Add open count and, if useful, an aging-tail threshold as a diagnostic only, not a target.
:::
...
:::
### Read together
Review MTTR with backlog age and the aging tail visible.
:::
...
<div class="tool-note">Visualize flow, backlog age, and the tail over time</div>
...
<div class="tool-note">Summarize count, age, and tail at a glance</div>
...
:::
### Track MTTR + MOVA
Align metrics with your prioritization strategy.
:::
...
:::
### MTTR alone
Can optimize for one part of the system.
:::
:::

::: {.line-anchor .centered .decision-close}
Do not read low MTTR as progress without checking backlog age.
:::
