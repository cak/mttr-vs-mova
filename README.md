...
The central claim is simple: **MTTR can rise while exposure falls**. **MTTR is flow**: the age of work that got closed. **MOVA** (Mean Open Vulnerability Age) is stock: the age of the work still open right now. When a team finally closes older backlog, the age of closed work goes up, so MTTR looks worse even while the open backlog gets healthier.
...
- **MTTR is flow**: the age of work that got closed.
- **MOVA is stock**: the age of work still open.
- MTTR alone can reward the wrong outcome: fast recent closures while older exposure stays stranded.
...
The published site uses this structure:
...
This keeps local development output in `_output/` while publishing the talk to `docs/slides/`.
...
- [Shiny for Python](https://shiny.posit.co/py/) is useful for lightweight interactive exploration when testing metric definitions and backlog slices.
- [Polars](https://pola.rs/) handles the metric logic.
